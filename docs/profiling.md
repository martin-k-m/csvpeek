...
(unless you ask for `--limit`), and nothing depends on iteration order, so two
runs over the same file produce identical output.
...
padded with empty strings; extra fields beyond the header are ignored. A file
...
This is deliberately a closed list. Sentinels that some exports use — `-1`,
`9999`, `?`, `-` — are *not* treated as missing, because guessing wrong would
silently distort every statistic in the column.
...
Three consequences worth knowing, because each one surprises somebody:

- **A column of only `0` and `1` is `bool`, not `int`** — `bool` is tested first.
  A flag column is far more common than a genuinely two-valued integer, but if
  you needed the arithmetic, this is why the numeric statistics are absent.
- **A bare year stays numeric.** `date` requires a separator, so `2024` is an
  `int` and not a date. That keeps ids and years out of the date bucket.
...
`%m/%d/%Y` is tried before `%d/%m/%Y`, so an unambiguous-looking `03/04/2024` is
read as March 4th. csvpeek only reports *that the column is a date* — it never
reformats or re-emits the value — so the ambiguity does not propagate.
...
**`int` and `float` columns** additionally get, with all non-null values read as
floats:
...
Two details that matter if you compare csvpeek against another tool:
...
- `stdev` is the **population** deviation, not the sample one. It divides by *n*,
  ...
  undefined.
- `p25`/`p75` use Python's default **exclusive** quantile method, which
  interpolates and can therefore fall outside the range spanned by small samples'
  order statistics. They are omitted entirely for a column with fewer than two
  values.

**Everything else** (`string`, `bool`, `date`, `empty`) gets `top` instead: the
most common values as `(value, count)`, ordered by count descending and then by
value ascending. That secondary sort is what makes ties reproducible; without it
the output would depend on dictionary insertion order. `-t/--top` controls how
many are kept.
